...
为了保持 tag 的向后兼容，以及调用 KYC 的第三方智能合约的兼容性，supported tag 仅作为对新的修改操作的限制。

如果一次针对用户 tag 的改动中的 tag，不在 supported tag 中，那么操作失败。该操作包括对老用户的该 tag 的修改，和新用户对该tag的初次赋值。

对于已被取消的 supported tag 的 tag 的读取操作，不受限制。
